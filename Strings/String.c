...
string *new_string()
{
    ...
    if (!rv->ctr) {free(rv); return NULL;}

    rv->length = 0;
    rv->value = malloc(1);
    if (rv->value == NULL) {__free_ptr(rv->ctr); return NULL;}
    rv->v_ctr = add_to_be_freed(rv->value);
    if (!rv->v_ctr) {free(rv->value); __free_ptr(rv->ctr); return NULL;}
    rv->value[0] = 0;
    rv->end = rv->value;
    return rv;
}
...
bool assign_str(string *s, char *v)
{
    ...
    if (!s->v_ctr){free(s->value); return false;}
...
bool assign_concat_str_str(string *a, string *b)
{
    ...
    if (!a->v_ctr) {free(temp); return false;}
    a->value = temp;
    a->length += b->length;
...
bool clone_to(string *to, string *from)
{
    ...
    if (!to->v_ctr) {free(to->value); return false;}
...
bool contains_str(string *a, string *b)
{
    ...
    for (unsigned long x = 0, l = a->length - b->length + 1; x < l; x++)
    {
        ...
        for (; y < m; y++)
        {
            if (a->value[x + y] != b->value[y]) break;
        }
        if (y == m) return true;
    }
    return false;
...
bool ends_with_str(string *a, string *b)
{
    ...
    {
        if (a->value[tmp + x] != b->value[x]) return false;
    }
    return true;
}
bool ends_with_arr(string *a, char *b)
{
    ...
    {
        if (a->value[tmp + x] != b[x]) return false;
    }
    return true;
}
bool starts_with_str(string *a, string *b)
{
    ...
    for (unsigned long x = 0; x < b->length; x++)
    {
        if (a->value[x] != b->value[x]) return false;
    }
    return true;
}
bool starts_with_arr(string *a, char *b)
{
    ...
    for (unsigned long x = 0; x < b_len; x++)
    {
        if (a->value[x] != b[x]) return false;
    }
    return true;
}
string *insert_str(string *a, unsigned long index, string *b)
{
    if (a->length < index) return NULL;
    ...
    if (new->value == NULL){__free_ptr(new->ctr); return NULL;}
    new->v_ctr = add_to_be_freed(new->value);
    if (!new->v_ctr) {free(new->value); __free_ptr(new->ctr); return NULL;}
...
bool assign_insert_str(string *a, unsigned long index, string *b)
{
    ...
    if (!a->v_ctr) {free(a->value); return false;}
...
string *str_trim_start(string *a)
{
    ...
    if (a->value[0] != ' ') return clone_str(a);
    unsigned long f = 0;
    while (a->value[f] == ' ') {f++;}
    ...
    if (!rv->v_ctr) {free(n); __free_ptr(rv->ctr); return NULL;}
...
{
    if (a == NULL) return NULL;
    if (a->length <= 0) return new_string();
    else if (a->end[-1] != ' ') return clone_str(a);
...
{
    if (a == NULL) return false;
    if (!assign_str_trim_start(a)) return false;
    if (!assign_str_trim_end(a)) return false;
    return true;
}
string **str_split(string *a, char sep, unsigned long *length)
{
    if (a == NULL || length == NULL) return false;
    *length = 0;
    ...
    for (unsigned long i = 0; i <= a->length; i++)
    {
        if (a->value[i] == sep || a->value[i] == 0)
        {
            ...
            {
                free(x);
                if (!add_to_be_freed(rv)) {*length = 0; free(rv); return NULL;}
                return rv;
            }
            *length = *length + 1;
            string **x = realloc(rv, sizeof(string *) * (*length));
            if (x == NULL)
            {
                free(x);
                if (!add_to_be_freed(rv)) {*length = 0; free(rv); return NULL;}
...
string *str_to_lower(string *s)
{
    ...
    {
        if (s->value[i] >= 65 && s->value[i] <= 90) rv->value[i] += 32;
    }
    return rv;
}
...
string *str_to_upper(string *s)
{
    ...
    for (unsigned long i = 0; i < s->length; i++)
    {
        if (s->value[i] >= 97 && s->value[i] <= 122) rv->value[i] -= 32;
    }
    return rv;
}
...
{
    if (s->length < start + length) return NULL;
    string *rv = new_string();
    __free_ptr(rv->v_ctr);
    char *x = malloc(length + 1);
    if (x == NULL) {__free_ptr(rv->ctr); return NULL;}
    memcpy(x, &(s->value[start]), length);
    rv->v_ctr = add_to_be_freed(x);
    if (!rv->v_ctr) {free(x); __free_ptr(rv->ctr); return NULL;}
    rv->value = x;
    rv->end = rv->value + length;
    rv->length = length;
    return rv;
...
string *str_remove(string *s, unsigned long start, unsigned long length)
{
    ...
    memcpy(&(rv->value[start]), &(s->value[start + length]), length - start);
    rv->value[length] = 0;
    return rv;
}
...
{
    if (s->length < x->length) return s->length;
    for (unsigned long i = 0, l = s->length; i < l; i++)
    {
        unsigned long y = 0, m = x->length;
        for (; y < m; y++)
        {
            if (s->value[i + y] != x->value[y]) break;
        }
        if (y == m) return i;
    }
    return s->length;
...
unsigned long index_of_arr(string *s, char *x)
{
    ...
    if (s->length < x_len) return s->length;
    for (unsigned long i = 0, l = s->length; i < l; i++)
    {
        unsigned long y = 0, m = x_len;
        for (; y < m; y++)
        {
            if (s->value[i + y] != x[y]) break;
        }
        if (y == m) return i;
    }
    return s->length;
...
bool isPalindrome(string *s)
{
    ...
    {
        if (s->value[i] != s->value[s->length - 1 - i]) return false;
    }
    return true;
}
